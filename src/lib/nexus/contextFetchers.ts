...
import type { PortalType, UserContext } from './prompts';
...
export async function fetchUserContext(
  ...
  try {
    ...
    console.error('[contextFetchers] Error fetching context:', e);
    return baseContext;
  }
}
...
async function fetchClientContext(
  ...
  try {
    ...
    console.error('[contextFetchers] Client context error:', e);
  }
...
async function fetchCoachingContext(
  ...
  try {
    ...
    console.error('[contextFetchers] Coaching context error:', e);
  }
...
async function fetchCandidateContext(
  ...
  try {
    ...
    console.error('[contextFetchers] Candidate context error:', e);
  }
...
async function fetchInternalContext(
  ...
  try {
    ...
    console.error('[contextFetchers] Internal context error:', e);
  }

  return ctx;
}

export function determinePortalType(userContext: any): PortalType {
  if (userContext?.client_account_id) return 'client';
  if (userContext?.role === 'admin' || userContext?.role === 'consultant') return 'internal';
  if (userContext?.is_coachee) return 'coaching';
  return 'candidate';
}
