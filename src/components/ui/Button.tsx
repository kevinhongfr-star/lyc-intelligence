...
export function Button({ variant = 'default', size = 'default', children, className, ...props }: { variant?: keyof typeof variants; size?: keyof typeof sizes; children: React.ReactNode; className?: string } & React.ButtonHTMLAttributes<HTMLButtonElement>) {
  return <button className={cn('inline-flex items-center justify-center gap-2 font-medium rounded-lg transition-colors min-h-[44px] disabled:opacity-50 disabled:cursor-not-allowed', variants[variant], sizes[size], className)} {...props}>{children}</button>;
}
