...
import { BarChart3, Users, Briefcase, TrendingUp, CheckCircle2, Zap, Plus, Loader2, Activity, Clock, Award } from 'lucide-react';
import { Card, CardHeader, CardTitle, CardContent, Badge, Button } from '@/components/ui';
import { useAuth } from '@/contexts';
import { STAGE_CONFIG, STAGE_ORDER } from '@/types/mandate';
import { useDashboardStats, useMandates, useTierDistribution, useRecentActivity } from '@/hooks/useSupabaseData';
...
export function ConsultantDashboard() {
  ...
  const { data: tiers, loading: tiersLoading } = useTierDistribution();
  const { data: activity, loading: activityLoading } = useRecentActivity(15);
              ...
              <Card>
                ...
                <CardContent>
                  {activityLoading ? (
                    ...
                    activity.slice(0, 8).map((item, i) => <ActivityItem key={`${item.type}-${item.id}-${i}`} item={item} />)
                  )}
                </CardContent>
              </Card>
            </div>
