      ...
      {activeTab === 'overview' ? (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-secondary-500/20 flex items-center justify-center"><Briefcase className="w-5 h-5 text-secondary-400" /></div><div><p className="text-2xl font-bold text-text-primary">{statsLoading ? '—' : (stats?.mandatesByStatus?.['1_search'] ?? 0) + (stats?.mandatesByStatus?.['2_call'] ?? 0) + (stats?.mandatesByStatus?.['3_deliver'] ?? 0)}</p><p className="text-xs text-text-muted">Active Mandates</p></div></div></CardContent></Card>
            <Card><CardContent className="p-4"><div className="flex items-center gap-3"><div className="w-10 h-10 rounded-lg bg-secondary-500/20 flex items-center justify-center"><Users className="w-5 h-5 text-secondary-400" /></div><div><p className="text-2xl font-bold text-text-primary">{statsLoading ? '—' : stats?.totalContacts?.toLocaleString() ?? '0'}</p><p className="text-xs text-text-muted">Total Candidates</p></div></div></CardContent></Card>
