          ...
          return (
            <div key={stage} className="group">
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 h-8 bg-bg-alt overflow-hidden relative">
                  <div
            ...
            <div key={kpi.id} className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-0.5 text-xs font-medium ${statusColors[kpi.status] || 'bg-gray-100 text-gray-700'}`}>
                    {kpi.status}
                  </span>
                </div>
              </div>
          ...
          {mandates.slice(0, 5).map(mandate => {
            ...
            return (
              <div
                ...
                className={`p-3 border ${colors.bg} hover:shadow-sm transition-shadow cursor-pointer`}
              >
                <div className="flex items-start justify-between">
          ...
          items.slice(0, 15).map((item, idx) => {
            ...
            return (
              <div key={idx} className="flex items-start gap-3">
                <div className="w-8 h-8 bg-bg-alt flex items-center justify-center flex-shrink-0">
                  <Icon className="w-4 h-4 text-text-muted" />
                </div>
                <div className="flex-1 min-w-0">
    ...
    <div className="bg-card border border-border p-5">
      ...
      <div className="space-y-2">
        {consultants.map((c, idx) => (
          <div
            key={c.consultant_id}
            className="flex items-center gap-3 p-2 hover:bg-bg-alt/50 transition-colors"
          >
...
export function BottleneckAlert({ bottlenecks }: BottleneckAlertProps) {
  ...
  return (
    <div className={`p-4 border ${severityColors[bottlenecks[0].severity] || severityColors.warning}`}>
