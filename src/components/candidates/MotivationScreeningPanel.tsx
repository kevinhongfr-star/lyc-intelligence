      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {MOTIVATION_FACTORS.map((factor) => (
            <div key={factor.key} className="border p-3">
              {/* Factor Label */}
                ...
                {(['GREEN', 'YELLOW', 'RED', 'UNKNOWN'] as const).map((value) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-1 flex items-center justify-center gap-1 px-3 py-2 border transition-all ${ isSelected ?`${colors.bg} ${colors.border} ${colors.text}`: 'bg-bg-tertiary border-bg-tertiary text-text-secondary hover:bg-gray-50' }`}
                    >
                      {colors.icon}
