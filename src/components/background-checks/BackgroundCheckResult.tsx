...
export function BackgroundCheckResult({ check, onSave, onCancel }: BackgroundCheckResultProps) {
  ...
  return (
    <Card className="p-6">
      ...
      {/* Check Info */}
      <div className="p-4 bg-bg-alt mb-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div
            className={`mt-2 p-4 border-2 border-dashed text-center cursor-pointer transition-colors ${
            ...
            {RESULT_OPTIONS.map(option => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-2 p-3 border-2 transition-colors ${ formData.result === option.value ?`${option.bg} border-current ${option.color}`: 'border-border hover:border-text-muted' }`}
                >
