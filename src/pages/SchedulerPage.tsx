    ...
    <div className="space-y-6">
      {showCreate && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowCreate(false)}>
          <div className="bg-bg-secondary border border-bg-tertiary w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-serif font-bold text-text-primary">New Event</h3>
              <button onClick={() => setShowCreate(false)} className="p-2 hover:bg-bg-tertiary"><X className="w-4 h-4 text-text-muted" /></button>
            </div>
            <input value={newEvent.title} onChange={e => setNewEvent(p => ({ ...p, title: e.target.value }))} placeholder="Event title" className="w-full px-4 py-3 bg-bg-primary border border-bg-tertiary text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent" />
            <div className="grid grid-cols-2 gap-3">
              <input type="date" value={newEvent.date} onChange={e => setNewEvent(p => ({ ...p, date: e.target.value }))} className="px-4 py-3 bg-bg-primary border border-bg-tertiary text-sm text-text-primary focus:outline-none focus:border-accent" />
              <input type="time" value={newEvent.time} onChange={e => setNewEvent(p => ({ ...p, time: e.target.value }))} className="px-4 py-3 bg-bg-primary border border-bg-tertiary text-sm text-text-primary focus:outline-none focus:border-accent" />
            </div>
            <input value={newEvent.location} onChange={e => setNewEvent(p => ({ ...p, location: e.target.value }))} placeholder="Location (optional)" className="w-full px-4 py-3 bg-bg-primary border border-bg-tertiary text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-accent" />
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <button onClick={() => openCreateModal()} className="flex items-center gap-2 px-4 py-2 bg-accent hover:bg-accent-light text-white text-sm font-medium min-h-[44px]"><Plus className="w-4 h-4" />New Event</button>
          <button onClick={() => setView('month')} className={`px-3 py-2 text-sm min-h-[44px] ${view === 'month' ? 'bg-accent text-white' : 'bg-bg-tertiary text-text-muted'}`}>Month</button>
          <button onClick={() => setView('agenda')} className={`px-3 py-2 text-sm min-h-[44px] ${view === 'agenda' ? 'bg-accent text-white' : 'bg-bg-tertiary text-text-muted'}`}>Agenda</button>
        </div>
      </div>
      {view === 'month' ? (
          ...
          <div className="lg:col-span-2">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between">
                <button onClick={prevMonth} className="p-2 hover:bg-bg-tertiary min-h-[44px] min-w-[44px] flex items-center justify-center"><ChevronLeft className="w-4 h-4 text-text-muted" /></button>
                <CardTitle>{MONTHS[viewMonth]} {viewYear}</CardTitle>
                <button onClick={nextMonth} className="p-2 hover:bg-bg-tertiary min-h-[44px] min-w-[44px] flex items-center justify-center"><ChevronRight className="w-4 h-4 text-text-muted" /></button>
              </CardHeader>
              <CardContent>
                  ...
                  {weeks.map((week, wi) => week.map((date, di) => {
                    ...
                    return (
                      <button key={`${wi}-${di}`} onClick={() => setSelectedDate(date)} onDoubleClick={() => openCreateModal(date)}
                        ...
                        <span className={`text-xs font-medium ${isToday ? 'w-5 h-5 flex items-center justify-center bg-accent text-white' : 'text-text-primary'}`}>{date.getDate()}</span>
                        <div className="mt-1 space-y-0.5">{dayEvents.slice(0, 2).map((e, i) => <div key={i} className="text-[10px] px-1 py-0.5 bg-accent/20 text-accent truncate">{e.title}</div>)}</div>
                      </button>
                    );
                  }))}
                </div>
          ...
          <Card>
            ...
            <CardContent>
              {selectedEvents.length === 0 ? (
                ...
                <div key={e.id} className="p-3 bg-bg-tertiary mb-2">
        ...
        <Card>
          ...
          <CardContent>
            {upcomingEvents.length === 0 ? (
              ...
              <div key={e.id} className="flex items-center gap-4 p-3 bg-bg-tertiary mb-2">
