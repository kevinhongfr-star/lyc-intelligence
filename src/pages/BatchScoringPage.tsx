...
export function BatchScoringPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-serif font-bold text-text-primary">Batch TRIDENT Scoring</h1>
      ...
      <div className="flex items-center gap-4"><Button onClick={runScoring} disabled={scoring || !jd || candidates.every(c => !c.name)}>{scoring ? <><Loader2 className="w-4 h-4 animate-spin mr-2" />Scoring... ({progress}%)</> : <><Play className="w-4 h-4 mr-2" />Run TRIDENT Sweep</>}</Button>{scoring && <Progress value={progress} className="max-w-xs" />}</div>
