// ═══════════════════════════════════════════════════════════
// COACH Results Page — score-only, no archetype.
...
const ACCENT = '#059669';
const SESSION_KEY = 'assessment_answers_COACH_latest';
  ...
  return (
    <div style={{
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: GRAY_600, fontSize: 14 }}>Scoring your COACH results…</p>
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 480, textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 15, color: GRAY_600, lineHeight: 1.6, marginBottom: 32 }}>
          Your COACH results will appear here once you complete the assessment. It takes about eight minutes.
        </p>
        <Link
          to="/assessment/coach/take"
          style={{
        ...
        >
          Begin COACH assessment <ArrowRight style={{ width: 18, height: 18 }} />
        </Link>
      </div>
    </div>
  );
...
export function CoachResultsPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    async function load() {
      const scopedKey = `assessment_answers_COACH_${id}`;
      ...
      try {
        ...
        const out = await scoreAssessment('COACH', answers, { persist: false });
        if (!out.ok || cancelled) {
          if (!cancelled) setState({ loading: false, result: null, hasAnswers: false });
          return;
        }
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', width: '100%' }}>
        ...
        <div style={{
          ...
          >
            Back to COACH overview <ArrowRight style={{ width: 16, height: 16 }} />
          </Link>
        </div>
      </div>
    </div>
