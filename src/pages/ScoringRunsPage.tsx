    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          {runTypes.map(t => (
            <button key={t} onClick={() => { setFilterType(t); setPage(0); }} className={`px-3 py-2 text-sm min-h-[44px] ${filterType === t ? 'bg-accent text-white' : 'bg-bg-tertiary text-text-muted'}`}>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="w-2 h-2 flex-shrink-0" style={{ backgroundColor: RUN_TYPE_COLORS[run.run_type] ?? '#9CA3AF' }} />
                  ...
                  <button onClick={() => setExpandedId(expandedId === run.id ? null : run.id)} className="p-2 hover:bg-bg-tertiary text-text-muted min-h-[44px] min-w-[44px] flex items-center justify-center">
                  ...
                  <div className="mt-3 pt-3 border-t border-bg-tertiary space-y-2">
                    {run.input_params && (
                      <div>
                        ...
                        <pre className="text-xs text-text-secondary bg-bg-tertiary p-3 overflow-x-auto">{typeof run.input_params === 'string' ? run.input_params : JSON.stringify(run.input_params, null, 2)}</pre>
                      </div>
                    )}
                    {run.output_scores && (
                      <div>
                        <p className="text-xs text-text-muted mb-1">Output</p>
                        <pre className="text-xs text-text-secondary bg-bg-tertiary p-3 overflow-x-auto">{typeof run.output_scores === 'string' ? run.output_scores : JSON.stringify(run.output_scores, null, 2)}</pre>
                      </div>
                    )}
                  </div>
                )}
