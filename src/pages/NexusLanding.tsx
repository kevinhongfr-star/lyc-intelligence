...
const btnPrimary: React.CSSProperties = { ...btnBase, background: DS.text, color: DS.bg };
...
function WhatIs() {
  return (
    <section style={{ padding: '100px 0', background: DS.bgAlt }} className="section-padding">
      <div style={containerStyle} className="nl-reveal">
    ...
    <section id="capabilities" style={{ padding: '100px 0' }} className="section-padding">
      <div style={containerStyle}>
        ...
        <div className="nl-reveal grid-responsive" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, background: DS.border, border: `1px solid ${DS.border}` }}>
          {CAPS.map((c, i) => (
            <div key={c.n} style={{
              background: DS.bg, padding: '40px 36px', display: 'flex', flexDirection: 'column',
            ...
            onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'transparent'; }}>
              <div className="nl-reveal" style={{ width: 40, height: 40, marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'center', background: DS.bgAlt, fontFamily: DS.monoFont, fontSize: 14, color: DS.accent, fontWeight: 500, transitionDelay: `${i * 80}ms` }}>{c.n}</div>
              <h3 style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 22, marginBottom: 12, lineHeight: 1.25, color: DS.text }}>{c.title}</h3>
              <p style={{ color: DS.muted, fontSize: 15, lineHeight: 1.6, marginBottom: 20 }}>{c.desc}</p>
              <a href={c.href} style={{ fontFamily: DS.monoFont, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.08em', color: DS.accent, textDecoration: 'none', fontWeight: 500, marginTop: 'auto' }}
      ...
      <div style={containerStyle}>
        ...
        <div className="nl-reveal" style={{ maxWidth: 760, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 1, background: DS.border, border: `1px solid ${DS.border}` }}>
          {EXAMPLE_PROMPTS.map((q, i) => (
            <div key={i} style={{ background: DS.bg, padding: '22px 28px', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
              <span style={{ fontFamily: DS.monoFont, fontSize: 12, color: DS.accent, marginTop: 4, flexShrink: 0 }}>→</span>
              <span style={{ fontFamily: DS.headingFont, fontSize: 18, color: DS.text, lineHeight: 1.5, fontStyle: 'italic' }}>{q}</span>
            </div>
          ))}
        </div>
      </div>
...
function HowItWorks() {
  return (
    <section id="how-it-works" style={{ padding: '100px 0', background: DS.bgAlt }} className="section-padding">
      <div style={containerStyle}>
        <div className="nl-reveal" style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 64px' }}>
          <span style={sectionLabel}>How it works</span>
          <h2 className="section-heading" style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 36, lineHeight: 1.2, color: DS.text, marginBottom: 16 }}>Three steps. <em style={{ fontWeight: 400 }}>No guesswork.</em></h2>
          <p style={{ fontSize: 17, color: DS.muted, lineHeight: 1.6 }}>Trained on our assessment methodology and your results — NEXUS carries your context into every reply.</p>
        </div>
          ...
          {STEPS.map((s, i) => (
            <div key={s.n} style={{ paddingTop: 56, position: 'relative' }}>
              ...
              <div className="nl-reveal" style={{
                ...
                fontFamily: DS.headingFont, fontSize: 48, fontWeight: 400, fontStyle: 'italic', color: DS.accent, lineHeight: 1,
                transitionDelay: `${i * 80}ms`,
              }}>{s.n}</div>
              <h3 style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 20, marginBottom: 14, lineHeight: 1.25, color: DS.text }}>{s.title}</h3>
              <p style={{ color: DS.muted, fontSize: 15, lineHeight: 1.6 }}>{s.desc}</p>
            </div>
          ))}
        </div>
    ...
    <section id="diagnostics" style={{ padding: '100px 0' }} className="section-padding">
      <div style={containerStyle}>
        <div className="nl-reveal" style={{ maxWidth: 640, margin: '0 auto 56px', textAlign: 'center' }}>
          ...
          <h2 className="section-heading" style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 36, lineHeight: 1.2, color: DS.text, marginBottom: 20 }}>Know where you stand. <em style={{ fontWeight: 400 }}>Measured, not guessed.</em></h2>
          <p style={{ color: DS.muted, fontSize: 17, lineHeight: 1.6 }}>Every diagnostic is built on validated executive assessment frameworks and benchmarked against real leaders. Not personality tests. Actual leadership measurement.</p>
        </div>
        <div className="nl-reveal grid-responsive-2" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 1, background: DS.border, border: `1px solid ${DS.border}`, marginBottom: 56 }}>
          {DIAGS.map(d => (
            ...
            onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.borderColor = DS.accent; }}
            onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'transparent'; }}>
              <span style={{ fontFamily: DS.monoFont, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: DS.eyebrow, marginBottom: 14, display: 'block' }}>{d.tag}</span>
              <h4 style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 18, marginBottom: 10, lineHeight: 1.3, color: DS.text }}>{d.title}</h4>
              <p style={{ color: DS.muted, fontSize: 14, lineHeight: 1.6, marginBottom: 18 }}>{d.desc}</p>
              <div style={{ fontFamily: DS.monoFont, fontSize: 11, color: DS.mutedDim, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{d.meta}</div>
            </div>
          ))}
        </div>
        <div className="nl-reveal" style={{ textAlign: 'center' }}>
          <a href="#start" style={{ ...btnPrimary, background: DS.accent, borderColor: DS.accent }} {...ctaCompressHandlers}
    ...
    <section style={{ padding: '100px 0', background: DS.bgAlt }} className="section-padding">
      <div style={containerStyle}>
        <div className="nl-reveal" style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 64px' }}>
          ...
          <h2 className="section-heading" style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 36, lineHeight: 1.2, color: DS.text, marginBottom: 20 }}>Built for leaders <em style={{ fontWeight: 400 }}>who take growth seriously</em></h2>
          <p style={{ fontSize: 17, color: DS.muted, lineHeight: 1.6 }}>If you're ambitious and you know your development is the bottleneck, NEXUS is for you.</p>
        </div>
        <div className="nl-reveal grid-responsive" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32 }}>
          {PERSONAS.map(p => (
            <div key={p.title} style={{
              padding: '36px 28px', border: `1px solid ${DS.border}`, background: DS.bg,
              transition: 'transform 200ms cubic-bezier(0.4,0,0.2,1), border-color 200ms ease',
            }}
            onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.borderColor = DS.accent; }}
            onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = DS.border; }}>
              <span style={{ fontFamily: DS.monoFont, fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.1em', color: DS.eyebrow, marginBottom: 16, display: 'block' }}>{p.label}</span>
              <h4 style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 20, marginBottom: 12, lineHeight: 1.25, color: DS.text }}>{p.title}</h4>
              <p style={{ color: DS.muted, fontSize: 14, lineHeight: 1.6, marginBottom: 16 }}>{p.desc}</p>
              <ul style={{ listStyle: 'none', margin: 0, padding: 0 }}>
                {p.bullets.map(b => (
                  <li key={b} style={{ fontSize: 13, color: DS.muted, padding: '6px 0', borderBottom: `1px solid ${DS.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ color: DS.accent, fontSize: 11, fontWeight: 600 }}>→</span>{b}
                  </li>
                ))}
              </ul>
            </div>
...
function B2BTeaser() {
  return (
    <section id="for-business" style={{ textAlign: 'center', padding: '80px 0', background: DS.bgAlt, borderTop: `1px solid ${DS.border}`, borderBottom: `1px solid ${DS.border}` }} className="section-padding">
      <div style={containerStyle} className="nl-reveal">
        <span style={{ ...monoStyle, color: DS.muted, marginBottom: 16, display: 'block' }}>Also for business</span>
        <h3 style={{ fontFamily: DS.headingFont, color: DS.text, maxWidth: 500, margin: '0 auto 16px', fontSize: 26, fontWeight: 700, lineHeight: 1.25 }}>NEXUS for teams and organizations</h3>
        <p style={{ color: DS.muted, maxWidth: 500, margin: '0 auto 24px', fontSize: 15, lineHeight: 1.6 }}>If you're looking at this for your team or your firm, NEXUS scales for enterprise use. Team-level pipeline assessment, consistent scoring across recruiters, bulk candidate screening.</p>
...
function PricingCard({ plan }: { plan: typeof PLANS[number] }) {
  ...
  const bg = f ? DS.text : DS.bg;
  const text = f ? DS.bg : DS.text;
  const muted = f ? 'rgba(255,255,255,0.6)' : DS.muted;
  const border = f ? 'rgba(255,255,255,0.1)' : DS.border;
  const ctaStyle = f
    ? { ...btnPrimary, background: DS.accent, color: DS.bg, borderColor: DS.accent, width: '100%' as const }
    : { ...btnSecondary, width: '100%' as const, borderColor: DS.text, color: DS.text };
  return (
    <div style={{
    ...
    onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; if (!f) e.currentTarget.style.borderColor = DS.accent; }}
    onMouseLeave={(e) => { e.currentTarget.style.transform = 'translateY(0)'; if (!f) e.currentTarget.style.borderColor = 'transparent'; }}>
      <span style={{ fontFamily: DS.monoFont, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: DS.eyebrow, marginBottom: 12, fontWeight: 500 }}>{plan.label}</span>
      <h3 style={{ fontFamily: DS.headingFont, fontSize: 22, marginBottom: 8, fontWeight: 700, lineHeight: 1.25, color: text }}>{plan.title}</h3>
      <div style={{ fontFamily: DS.headingFont, fontSize: 28, fontWeight: 700, marginBottom: 20, color: text }}>
        {plan.price}{plan.small && <span style={{ fontSize: 14, fontWeight: 400, color: muted, fontFamily: DS.bodyFont }}> {plan.small}</span>}
      </div>
      ...
      <ul style={{ listStyle: 'none', flexGrow: 1, marginBottom: 28, padding: 0 }}>
        {plan.bullets.map((b, i) => (
          <li key={i} style={{ padding: '8px 0', fontSize: 14, borderBottom: i === plan.bullets.length - 1 ? 'none' : `1px solid ${border}`, color: f ? 'rgba(255,255,255,0.8)' : DS.muted }}>{b}</li>
        ))}
      </ul>
      <a href={plan.href} style={ctaStyle} {...ctaCompressHandlers}
        onMouseEnter={(e) => {
          if (f) { e.currentTarget.style.opacity = '0.9'; } else { e.currentTarget.style.background = DS.text; e.currentTarget.style.color = DS.bg; }
        }}
        onMouseLeave={(e) => {
          if (f) { e.currentTarget.style.opacity = '1'; } else { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = DS.text; }
        }}>
        {plan.cta}
      </a>
    </div>
    ...
    <section id="pricing" style={{ padding: '100px 0' }} className="section-padding">
      <div style={containerStyle}>
        <div className="nl-reveal" style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 64px' }}>
          ...
          <h2 className="section-heading" style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 36, lineHeight: 1.2, color: DS.text, marginBottom: 20 }}>Start with Executive Introduction. <em style={{ fontWeight: 400 }}>Grow when you're ready.</em></h2>
          <p style={{ fontSize: 17, color: DS.muted, lineHeight: 1.6 }}>Complimentary access includes 3 NEXUS messages to try it. Subscribers get unlimited NEXUS access.</p>
        </div>
        <div className="grid-responsive" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 1, background: DS.border, border: `1px solid ${DS.border}` }}>
          {PLANS.map(p => <PricingCard key={p.title} plan={p} />)}
        </div>
        <div style={{ textAlign: 'center', marginTop: 32 }}>
          <a href="/pricing" style={{ fontFamily: DS.monoFont, fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.1em', color: DS.accent, textDecoration: 'none', fontWeight: 500 }}
            onMouseEnter={(e) => (e.currentTarget.style.textDecoration = 'underline')}
    ...
    <section style={{ padding: '100px 0', background: DS.bgAlt }} className="section-padding">
      <div style={containerStyle}>
        <div className="nl-reveal" style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 64px' }}>
          ...
          <h2 className="section-heading" style={{ fontFamily: DS.headingFont, fontWeight: 700, fontSize: 36, lineHeight: 1.2, color: DS.text, marginBottom: 0 }}>Built on real expertise. <em style={{ fontWeight: 400 }}>Validated by real data.</em></h2>
        </div>
        <div className="nl-reveal grid-responsive" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 48, textAlign: 'center' }}>
          {TRUST.map(t => {
            ...
            <div key={t.title} style={{ padding: '0 16px' }}>
              <div style={{ width: 48, height: 48, margin: '0 auto 20px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: `1px solid ${DS.border}`, color: DS.accent, background: DS.bg }}>
                <Icon style={{ width: 22, height: 22, strokeWidth: 1.75 }} aria-hidden="true" />
              </div>
              <h4 style={{ fontFamily: DS.headingFont, fontSize: 18, marginBottom: 10, fontWeight: 700, color: DS.text }}>{t.title}</h4>
              <p style={{ color: DS.muted, fontSize: 14, lineHeight: 1.6 }}>{t.desc}</p>
            </div>
            );
          })}
        </div>
...
function FinalCTA() {
  return (
    <section id="start" style={{ textAlign: 'center', padding: '120px 0', background: DS.text, color: DS.bg }} className="section-padding">
      <div style={containerStyle} className="nl-reveal">
        <span style={{ ...monoStyle, color: DS.mutedDim, marginBottom: 20, display: 'block' }}>Ready when you are</span>
        <h2 className="section-heading" style={{ fontFamily: DS.headingFont, color: DS.bg, maxWidth: 640, margin: '0 auto 20px', fontSize: 42, fontWeight: 700, lineHeight: 1.15 }}>Leadership isn't a title.<br /><em style={{ fontWeight: 400 }}>It's a trajectory.</em></h2>
        ...
        <Link to="/nexus/chat" style={{ ...btnPrimary, background: DS.accent, color: DS.bg, borderColor: DS.accent, padding: '16px 36px' }} {...ctaCompressHandlers}
          onMouseEnter={(e) => (e.currentTarget.style.opacity = '0.9')}
          onMouseLeave={(e) => (e.currentTarget.style.opacity = '1')}>
...
function Footer() {
  ...
  return (
    <footer style={{ background: DS.bgAlt, padding: '64px 0 32px', borderTop: `1px solid ${DS.border}` }} className="section-padding">
      <div style={containerStyle}>
