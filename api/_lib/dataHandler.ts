      ...
      const [statsResult, mandatesResult, tiersResult, activityResult] = await Promise.all([
        (async () => {
          const [contacts, mandates, companies, proposals] = await Promise.all([
            db.selectMany('contacts', { select: 'id,seniority', limit: 10000 }),
            db.selectMany('mandates', { select: 'id,status', limit: 10000 }),
            db.selectMany('companies', { select: 'id', limit: 1 }),
            db.selectMany('proposals', { select: 'id', limit: 1 }),
          ]);
        ...
        (async () => {
          const contacts = await db.selectMany('contacts', { 
