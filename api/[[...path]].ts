...
export default async function handler(
  ...
  try {
    switch (module) {
      ...
      // ── Feishu Sync Hooks ──
      case 'feishu-sync': {
        const m = await import('./_lib/feishuSyncHandler.js');
        (req.query as any).path = pathArr.slice(1);
        return m.handler(req, res);
      }

      // ── T2 v2 API (Business Rules Engine) ──
      case 'v2': {
        const m = await import('./_lib/t2RulesEngine.js');
        (req.query as any).path = pathArr.slice(1);
        return m.handler(req, res);
      }

      // ── T3 v3 API (Dashboard & Analytics) ──
      case 'v3': {
        const m = await import('./_lib/t3DashboardHandler.js');
        (req.query as any).path = pathArr.slice(1);
        return m.handler(req, res);
      }
