/**
 * Minimal Supabase REST client for Vercel serverless functions.
 * Uses fetch + Supabase REST API (PostgREST) instead of @supabase/supabase-js
 * to avoid Vite bundling incompatibilities.
 */

const SUPABASE_URL = process.env.SUPABASE_URL || '';
const SUPABASE_SERVICE_ROLE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || '';
const SUPABASE_ANON_KEY = process.env.VITE_SUPABASE_ANON_KEY || '';

// ─── Auth ───────────────────────────────────────────────────────────

export async function authGetUser(token: string): Promise<{ id: string; email: string | null } | null> {
  try {
    const res = await fetch(`${SUPABASE_URL}/auth/v1/user`, {
      headers: {
        'Authorization': `Bearer ${token}`,
        'apikey': SUPABASE_ANON_KEY,
        'Content-Type': 'application/json',
      },
    });
    if (!res.ok) return null;
    const data = await res.json();
    return { id: data.id, email: data.email ?? null };
  } catch {
    return null;
  }
}

// ─── RPC ────────────────────────────────────────────────────────────

export async function rpc(functionName: string, params: Record<string, any>): Promise<{ data: any | null; error: any | null }> {
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/rpc/${functionName}`, {
      method: 'POST',
      headers: {
        'apikey': SUPABASE_SERVICE_ROLE_KEY,
        'Authorization': `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
        'Content-Type': 'application/json',
        'Prefer': 'return=representation',
      },
      body: JSON.stringify(params),
    });
    if (!res.ok) {
      const err = await res.json().catch(() => ({ message: res.statusText }));
      return { data: null, error: err };
    }
    const data = await res.json();
    return { data, error: null };
  } catch (error: any) {
    return { data: null, error: { message: error.message } };
  }
}

// ─── Shared helpers ─────────────────────────────────────────────────

function baseHeaders(extra?: Record<string, string>): Record<string, string> {
  return {
    'apikey': SUPABASE_SERVICE_ROLE_KEY,
    'Authorization': `Bearer ${SUPABASE_SERVICE_ROLE_KEY}`,
    'Content-Type': 'application/json',
    ...extra,
  };
}

function parseContentRange(header: string | null): number | null {
  if (!header) return null;
  const m = header.match(/\d+-\d+\/(\d+)/);
  return m ? parseInt(m[1], 10) : null;
}

// ─── Select query builder ───────────────────────────────────────────

type FilterOp = 'eq' | 'neq' | 'gt' | 'gte' | 'lt' | 'lte' | 'like' | 'ilike' | 'is' | 'in';

class SupabaseQuery {
  private table: string;
  private selectCols: string = '*';
  private filters: Array<{ col: string; op: FilterOp; val: string }> = [];
  private orderCol: string | null = null;
  private orderAsc: boolean = true;
  private limitN: number | null = null;
  private offsetN: number = 0;
  private wantCount: boolean = false;

  constructor(table: string) { this.table = table; }

  select(columns?: string, options?: { count?: 'exact' | 'planned' | 'estimated' }): this {
    if (columns !== undefined) this.selectCols = columns || '*';
    if (options?.count) this.wantCount = true;
    return this;
  }

  eq(column: string, value: any): this { this.filters.push({ col: column, op: 'eq', val: encodeURIComponent(String(value)) }); return this; }
  neq(column: string, value: any): this { this.filters.push({ col: column, op: 'neq', val: encodeURIComponent(String(value)) }); return this; }
  gt(column: string, value: any): this { this.filters.push({ col: column, op: 'gt', val: encodeURIComponent(String(value)) }); return this; }
  gte(column: string, value: any): this { this.filters.push({ col: column, op: 'gte', val: encodeURIComponent(String(value)) }); return this; }
  lt(column: string, value: any): this { this.filters.push({ col: column, op: 'lt', val: encodeURIComponent(String(value)) }); return this; }
  lte(column: string, value: any): this { this.filters.push({ col: column, op: 'lte', val: encodeURIComponent(String(value)) }); return this; }
  is(column: string, value: any): this { this.filters.push({ col: column, op: 'is', val: String(value) }); return this; }

  in(column: string, values: any[]): this {
    const encoded = values.map(v => {
      const s = String(v);
      // In PostgREST, in.(val1,val2) — values with commas need quoting
      if (s.includes(',') || s.includes('"')) {
        return `"${s.replace(/"/g, '""')}"`;
      }
      return s;
    }).join(',');
    this.filters.push({ col: column, op: 'in', val: `(${encoded})` });
    return this;
  }

  order(column: string, options?: { ascending?: boolean }): this {
    this.orderCol = column;
    this.orderAsc = options?.ascending ?? true;
    return this;
  }

  limit(n: number): this { this.limitN = n; return this; }

  range(from: number, to: number): this {
    this.offsetN = from;
    this.limitN = to - from + 1;
    return this;
  }

  private buildUrl(): string {
    const params = new URLSearchParams();
    params.set('select', this.selectCols);
    for (const f of this.filters) {
      params.set(f.col, `${f.op}.${f.val}`);
    }
    if (this.orderCol) {
      params.set('order', `${this.orderCol}.${this.orderAsc ? 'asc' : 'desc'}`);
    }
    if (this.limitN !== null) params.set('limit', String(this.limitN));
    if (this.offsetN > 0) params.set('offset', String(this.offsetN));
    return `${SUPABASE_URL}/rest/v1/${this.table}?${params.toString()}`;
  }

  async execute(): Promise<{ data: any[] | null; error: any | null; count?: number | null }> {
    try {
      const prefer = this.wantCount ? { 'Prefer': 'count=exact' } : {};
      const res = await fetch(this.buildUrl(), { headers: baseHeaders(prefer) });
      const count = parseContentRange(res.headers.get('content-range'));
      if (!res.ok) {
        const err = await res.json().catch(() => ({ message: res.statusText }));
        return { data: null, error: err, count };
      }
      const data = await res.json();
      return { data, error: null, count };
    } catch (error: any) {
      return { data: null, error: { message: error.message }, count: null };
    }
  }

  // Awaitable — resolves with standard supabase shape
  then<TResult>(
    onfulfilled?: (value: { data: any[] | null; error: any | null; count?: number | null }) => TResult | PromiseLike<TResult>,
    onrejected?: (reason: any) => TResult | PromiseLike<TResult>,
  ): Promise<TResult> {
    return this.execute().then(onfulfilled, onrejected);
  }

  async maybeSingle(): Promise<{ data: any | null; error: any | null }> {
    this.limitN = 1;
    const result = await this.execute();
    if (result.error) return { data: null, error: result.error };
    return { data: result.data?.[0] ?? null, error: null };
  }

  async single(): Promise<{ data: any | null; error: any | null }> {
    return this.maybeSingle();
  }
}

// ─── Mutation builder (insert/update/delete/upsert) ─────────────────

class SupabaseMutator {
  private table: string;
  private method: 'POST' | 'PATCH' | 'DELETE' = 'POST';
  private bodyData: any = null;
  private filters: Array<{ col: string; op: FilterOp; val: string }> = [];
  private selectCols: string = '*';
  private preferParts: string[] = ['return=representation'];
  private onConflictCol: string | null = null;
  private isUpsert: boolean = false;

  constructor(table: string) { this.table = table; }

  eq(column: string, value: any): this {
    this.filters.push({ col: column, op: 'eq', val: encodeURIComponent(String(value)) });
    return this;
  }

  select(columns?: string): this {
    if (columns !== undefined) this.selectCols = columns || '*';
    return this;
  }

  // Initiate insert (chain continues with .select() before await)
  insert(records: any, options?: { count?: 'exact' }): this {
    this.method = 'POST';
    this.bodyData = records;
    if (options?.count) this.preferParts.push(`count=${options.count}`);
    return this;
  }

  upsert(records: any, options?: { onConflict?: string; count?: 'exact' }): this {
    this.method = 'POST';
    this.bodyData = records;
    this.isUpsert = true;
    this.onConflictCol = options?.onConflict || null;
    // PostgREST upsert: Prefer: resolution=merge-duplicates + ?on_conflict=col
    this.preferParts = ['return=representation', 'resolution=merge-duplicates'];
    if (options?.count) this.preferParts.push(`count=${options.count}`);
    return this;
  }

  update(record: Record<string, any>, options?: { count?: 'exact' }): this {
    this.method = 'PATCH';
    this.bodyData = record;
    if (options?.count) this.preferParts.push(`count=${options.count}`);
    return this;
  }

  delete(options?: { count?: 'exact' }): this {
    this.method = 'DELETE';
    this.bodyData = null;
    if (options?.count) this.preferParts.push(`count=${options.count}`);
    return this;
  }

  private buildUrl(): string {
    const params = new URLSearchParams();
    params.set('select', this.selectCols);
    for (const f of this.filters) {
      params.set(f.col, `${f.op}.${f.val}`);
    }
    if (this.isUpsert && this.onConflictCol) {
      params.set('on_conflict', this.onConflictCol);
    }
    const qs = params.toString();
    return `${SUPABASE_URL}/rest/v1/${this.table}${qs ? '?' + qs : ''}`;
  }

  async execute(): Promise<{ data: any[] | null; error: any | null; count?: number | null }> {
    try {
      const res = await fetch(this.buildUrl(), {
        method: this.method,
        headers: baseHeaders({ 'Prefer': this.preferParts.join(', ') }),
        body: this.bodyData !== null ? JSON.stringify(this.bodyData) : undefined,
      });
      const count = parseContentRange(res.headers.get('content-range'));
      if (!res.ok) {
        const err = await res.json().catch(() => ({ message: res.statusText }));
        return { data: null, error: err, count };
      }
      let data: any[] = [];
      if (res.status !== 204) {
        const body = await res.json();
        data = Array.isArray(body) ? body : [body];
      }
      return { data, error: null, count };
    } catch (error: any) {
      return { data: null, error: { message: error.message }, count: null };
    }
  }

  then<TResult>(
    onfulfilled?: (value: { data: any[] | null; error: any | null; count?: number | null }) => TResult | PromiseLike<TResult>,
    onrejected?: (reason: any) => TResult | PromiseLike<TResult>,
  ): Promise<TResult> {
    return this.execute().then(onfulfilled, onrejected);
  }
}

// ─── Main client factory ────────────────────────────────────────────

export function createClient() {
  return {
    from(table: string) {
      return {
        select: (columns?: string, options?: { count?: 'exact' }) => {
          const q = new SupabaseQuery(table);
          return q.select(columns, options);
        },
        insert: (records: any, options?: { count?: 'exact' }) => {
          const m = new SupabaseMutator(table);
          return m.insert(records, options);
        },
        upsert: (records: any, options?: { onConflict?: string; count?: 'exact' }) => {
          const m = new SupabaseMutator(table);
          return m.upsert(records, options);
        },
        update: (record: Record<string, any>, options?: { count?: 'exact' }) => {
          const m = new SupabaseMutator(table);
          return m.update(record, options);
        },
        delete: (options?: { count?: 'exact' }) => {
          const m = new SupabaseMutator(table);
          return m.delete(options);
        },
      };
    },
    rpc: rpc,
    auth: {
      getUser: async (token: string) => {
        const user = await authGetUser(token);
        if (!user) return { data: { user: null }, error: { message: 'Invalid token' } };
        return { data: { user }, error: null };
      },
    },
  };
}

export default { createClient };
