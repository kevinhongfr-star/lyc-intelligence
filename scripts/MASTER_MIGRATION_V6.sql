...
INSERT INTO public.sourcing_channels (channel_key, channel_label, is_active) VALUES
  ...
  ('liepin',   'Liepin (??)', FALSE),
  ('zhilian',  'Zhilian (??)', FALSE),
  ('zhipin',   'Zhipin (BOSS ??)', FALSE)
...
CREATE INDEX IF NOT EXISTS idx_car?_candidate_id    ON public.candidate_assessment_responses (candidate_id);
CREATE INDEX IF NOT EXISTS idx_car?_assessment_id   ON public.candidate_assessment_responses (assessment_id);
...
--  7. S2S3 GATE VALIDATION FUNCTION (GRID v2.0) 
...
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '?? Mandate extension error (may be already applied): %', SQLERRM;
END$$;
...
BEGIN
  ...
  END IF;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE '?? Contact extension error: %', SQLERRM;
END$$;
